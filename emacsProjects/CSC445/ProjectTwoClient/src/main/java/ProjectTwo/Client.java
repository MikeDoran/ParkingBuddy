...
public class Main {
    ...
    public static void main(String [] args){
        ...
        DatagramPacket wrq = createWriteRequest(fileName, PORT, hostIP);
        try {
    ...
    //            ------------------------------------------------
    //                            RRQ/WRQ packet
    private static DatagramPacket createWriteRequest(String filename, int port, InetAddress ip) {
        byte [] nameBytes = new byte[0];
        byte [] modeBytes = new byte[0];
        try {
            nameBytes = filename.getBytes("UTF-16");
            modeBytes = "octet".getBytes("UTF-16");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            System.exit(-1);
        }

        char nameLength = (char)nameBytes.length;
        char modeLength = (char)modeBytes.length;
        byte [] m = new byte [6 + nameLength + modeLength];
